...
const getUsersByGroupAndFieldType = async (req, res) => {
 ...
 const subscribeToSeries = async (req, res) => {
  try {
    ...
    if (!user.subscriptions.includes(seriesId)) {
      user.subscriptions.push(seriesId);
      await user.save();
 ...
 const unsubscribeFromSeries = async (req, res) => {
  try {
    const { userId, seriesId } = req.body;
    ...
    const updatedSubscriptions = user.subscriptions.filter(id => id.toString() !== seriesId);
    
    // Check if user was actually subscribed
    if (updatedSubscriptions.length === user.subscriptions.length) {
      return res.status(400).json({ error: "User is not subscribed to this series" });
    }
